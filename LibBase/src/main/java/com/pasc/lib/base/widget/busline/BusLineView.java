package com.pasc.lib.base.widget.busline;
...
public class BusLineView extends View {
    ...
    private Drawable mBus, mBusSmall, mCurrentPosition, mCurrentPositionLocation, mRailway, mStationPosition;
    private int mMargin1X, mMargin2P;
    private float mStrokeWidth;
    private int mColorPass, mColorUnreach, mSelectedColor, mColorBlock, mColorBusy, mColorNormal, mNubTextColor, mBusTextColor;
    ...
    private void init(Context context) {
        ...
        mBus = getResources().getDrawable(R.drawable.ic_directions_bus);
        mBusSmall = getResources().getDrawable(R.drawable.ic_directions_bus);
        mCurrentPosition = getResources().getDrawable(R.drawable.ic_circle_current_directing);
        mStationPosition = getResources().getDrawable(R.drawable.ic_circle_directing);
        mCurrentPositionLocation = getResources().getDrawable(R.drawable.ic_current_loaction);
        mRailway = getResources().getDrawable(R.drawable.ic_railway);

        mBusStationWidth = mBus.getIntrinsicWidth() << 1;
        float density = getResources().getDisplayMetrics().density;
        mMargin1X = (int) (6 * density);
        mMargin2P = (int) (2 * density);

        mStationCirclePaint = new Paint();
        mStrokeWidth = 6 * density;
        ...
        mColorPass = Color.parseColor("#4A63BA");//环的颜色
        mColorUnreach = Color.parseColor("#C0C0C0");
        mSelectedColor = Color.parseColor("#FF6658");

        mColorBlock = Color.parseColor("#F41C0D");
        mColorBusy = Color.parseColor("#FFAC00");
        mColorNormal = Color.parseColor("#00BD00");

        mNubTextColor = Color.parseColor("#6A6A6A");
        mBusTextColor = Color.parseColor("#6A6A6A");

        mTxtRect = new Rect();
        mBusRect = new RectF();
        mLocationRectF = new RectF();
        mNumTxtSize = sp2px(12);
        mStationNameTxtSize = sp2px(14);
        mSelectedTxtSize = sp2px(14);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0, size = mBusLines.size(); i < size; ++i) {
            ...
            if (item.isCurrentPosition) {
                ...
                canvas.drawBitmap(((BitmapDrawable) mStationPosition).getBitmap(), pos - mStationPosition.getIntrinsicWidth() / 2, offsetY - mStrokeWidth / 2 + mStationPosition.getIntrinsicWidth() / 2 + SizeUtils.dp2px(1.4f), null);
            ...
            mTxtPaint.setColor(item.isCurrentPosition ? mSelectedColor : mNubTextColor);
            mTxtPaint.setTextSize(mNumTxtSize);
            measureText(s, mTxtRect, mTxtPaint);
            canvas.drawText(s, pos - (mTxtRect.width() >> 1), offsetY + (margin2X - mTxtRect.height()) / 2, mTxtPaint);

            // 名称
            mTxtPaint.setColor(item.isCurrentPosition ? mSelectedColor : mBusTextColor);
            mTxtPaint.setTextSize(item.isCurrentPosition ? mSelectedTxtSize : mStationNameTxtSize);
            offsetY += mTxtRect.height() + margin2X;
            int oy = offsetY;
