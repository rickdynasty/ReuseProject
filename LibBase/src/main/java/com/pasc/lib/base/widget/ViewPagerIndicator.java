...
/**
 * Created by zhangcan603 on 2018/3/27.
 */
...
public class ViewPagerIndicator extends View implements ViewPager.OnPageChangeListener {
    ...
    public ViewPagerIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray =
                getResources().obtainAttributes(attrs, R.styleable.ViewPagerIndicatorView);
        //获取自定义属性的个数
        int N = typedArray.getIndexCount();
        for (int i = 0; i < N; i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.ViewPagerIndicatorView_indicator_icon) {
                //通过自定义属性拿到指示器
                mIndicator = typedArray.getDrawable(attr);
            } else if (attr == R.styleable.ViewPagerIndicatorView_indicator_margin) {
                float defaultMargin =
                        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5,
                                getResources().getDisplayMetrics());
                mMargin = (int) typedArray.getDimension(attr, defaultMargin);
            } else if (attr == R.styleable.ViewPagerIndicatorView_indicator_smooth) {
    ...
    private int measureWidth(int widthMeasureSpec) {
        ...
        int width;
        int desired = getPaddingLeft() + getPaddingRight() + mIndicatorSize * mCount + mMargin * (
                mCount
                        - 1);
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        Log.v("zhangcan", "========" + position + ",===offset" + positionOffset);
        if (mSmooth) {
